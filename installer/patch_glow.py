...
def patch_file(path: Path, airframe: str, dry: bool = False, reverse: bool = False) -> int:
    ...
    ⚠ `newline=""` IS LOAD-BEARING. ToLiss's mesh OBJs are CRLF, and the intended
    change here is ONE TOKEN PER REGION — five lines on the A339. A plain
    `read_text()` universal-newlines the file to LF on the way in, so the write-back
    silently reformats every line ending in a multi-megabyte file we are otherwise
    barely touching, and the backup/restore round trip stops being byte-exact.
    Found by `tests/test_parity.py` (2026-08-07)."""
...
def main():
    """The dev CLI (`python build/patch_glow.py --root … --airframe …`) — see
    patch_acf.main for why this is a public function."""
